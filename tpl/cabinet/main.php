	...
	<template id="editProfile" wb-off>
		<form class="profile-edit d-block" on-submit="save">
			...
			<div class="row profile-edit__wrap">
				...
				<div class="col-md-3">
					<div class="input input--grey">
						<input class="input__control" type="email" name="email" required value="{{ email }}" placeholder="E-mail">
			...
			<div class="select status">
				...
				<div class="select__list">
					...
					{{#each catalog.quoteStatus}}
						...
						{{else}}
							<div class="select__item select__item--acc-{{color}}" data-id="{{id}}" data-group="{{type}}" onclick="$(this).parents('.select.status').find('input.status').val($(this).attr('data-id'));$(this).parents('.select.status').find('input.group').val($(this).attr('data-group'));">
								{{name}}
							</div>
						{{/if}}
					{{/each}}
			...
			<div class="row">
				...
				<div class="col-md-4 --jcfe --flex" style="flex-direction: column-reverse; align-items: flex-end;">
					<div style="width: 100%; max-width: 305px;">
						...
						<textarea class="admin__editor-textarea" name="comment_for_expert" placeholder="Комментарий для специалистов">{{record.comment_for_expert}}</textarea>
					</div>
					<div style="width: 100%; max-width: 305px;">
							...
							<div class="acount__table-accardeon accardeon acount__table-accardeon--pmin status-{{status}}" data-client="{{record.client}}" data-record="{{record.id}}" data-id="{{record.id}}" data-idx="{{idx}}" data-priority="{{record.priority}}" data-group="{{record.group}}">
								<div class="acount__table-main accardeon__main acount__table-auto">
									<div class="admin-events-item heap">
										<div class="accardeon__click" data-record="{{record.id}}" data-idx="{{idx}}" on-click="toggleAccordeon"></div>
										<div class="flag-date">
											{{#if priority * 1}}
