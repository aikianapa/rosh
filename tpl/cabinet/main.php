...
<template id="listEvents" wb-off>
    <div class="account-scroll">
        ...
        <ul class="pagination custom">
            <li class="page-item" data-page="1" on-click="onPage">
                &laquo;
            </li>
            {{#pages}}
            <li class="page-item number" data-page="{{.}}" on-click="onPage">{{.}}</li>
            {{/pages}}
            <li class="page-item interim">
                ...
            </li>
            <li class="page-item" data-page="{{countPages}}" on-click="onPage">
                &raquo;
            </li>
        </ul>
        {{/if}}
...
<script wb-app remove>
    ...
    $(document).on('cabinet-db-ready', function () {
        ...
        tab_urls.forEach(tab => {
            sessionStorage.setItem(tab + '?page', 1);
        })
                            ...
                            _tab = new Ractive({
                                ...
                                on: {
                                    ...
                                    complete(ev) {
                                        ...
                                        switchPagination(pages, target_tab);
                                    },
                                    onPage(link) {
                                        link.event.preventDefault();
                                        const numberPage = link.node.dataset.page;
                                        ...
                                        utils.api.get('/api/v2/list/records?' + target_tab, {
                                            ...
                                            '@page': numberPage
                                                ...
                                                _tab.set("records", result.result)
                                                _tab.set("page", numberPage)

                                                sessionStorage.setItem(target_tab + '?page', numberPage);
                            ...
                            _tab = new Ractive({
                                ...
                                on: {
                                    ...
                                    complete(ev) {
                                        ...
                                        console.log("pageRecords:", _tab.get("records"))
        ...
        $('body').on('click', '.account__tab-items .account__tab-item.data-tab-link', function (e, prevent_editor_reset) {
            ...
            sessionStorage.setItem('state.tab-cabinet.type', _type);
            autoupdate_stop();
            window.can_update = false;
            ...
            autoupdateTimer = setTimeout(function reload() {
                try {
                    if (window.can_update) {
                        ...
                        window.load(sessionStorage.getItem('state.tab-cabinet.type'));
                    }
                } finally {
                    autoupdateTimer = setTimeout(reload, 5000);
                }
            }, exec_immediately ? 100 : 5000);
        }


        /* function getRecords(page, data, pages, target_tab, _sort) {
             utils.api.get('/api/v2/list/records?' + target_tab, {
                 '@sort': _sort,
                 '@size': 10,
                 '@page': sessionStorage.getItem(target_tab + '?page')
        ...
        function switchPagination(pages, target_tab) {
            const page = parseFloat(sessionStorage.getItem(target_tab + "?page"));
            if (pages > 5) {
                const interim = document.querySelector('.data-tab-item[data-type="' + target_tab + '"] ' + ".interim");

                [...document.querySelectorAll('.data-tab-item[data-type="' + target_tab + '"] ' + ".pagination .page-item.number")].forEach(btn => {
                    const btnPage = Number(btn.dataset.page);
                    const isActivePage = btnPage === page;
                    ...
                    if (page !== 1) {
                        const isNearPage = Math.abs(btnPage - page) <= 1;

                        if (isNearPage || btnPage > pages - 2) {
                ...
                [...document.querySelectorAll('.data-tab-item[data-type="' + target_tab + '"] ' + ".pagination .page-item.number")].forEach(btn => {
                    ...
                    if (isActivePage) btn.classList.add("active")
