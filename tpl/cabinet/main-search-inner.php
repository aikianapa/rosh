					...
					<div class="account__table-body">
						...
						{{#each history.events as event}}
							<div class="acount__table-accardeon accardeon" data-idx="{{@index}}">
								<div class="acount__table-main accardeon__main accardeon__click" data-record="{{this.id}}" data-idx="{{@index}}" on-click="['editPastRecord', this]">
								...
								<div class="row">
									<div class="col-md-12">
										<div class="input input-lk-calendar input--grey">
											<input class="input__control datepickr empty-date" name="event_date" data-date="{{record.event_date}}" value="{{ @global.utils.dateForce(record.event_date) }}" autocomplete="off" type="text" placeholder="Выбрать дату и время">
								...
								<div class="select status">
									...
									<div class="select__list">
										...
										{{#each @global.catalog.quoteStatus}}
											...
											{{else}}
												<div class="select__item select__item--acc-{{color}}" data-id="{{id}}" data-group="{{type}}" onclick="$(this).parents('.select.status').find('input.status').val($(this).attr('data-id'));$(this).parents('.select.status').find('input.group').val($(this).attr('data-group'));$(this).parents('.select.status').addClass('has-values')">
													{{name}}
												</div>
											{{/if}}
										{{/each}}
			...
			<div class="row">
				<div class="col-md-4">
					...
					{{#each photos.before}} <!--single photo!-->
						<a class="after-healing__item" data-fancybox="images" href="{{.src}}" data-caption="Фото до начала лечения, {{.date}}">
		...
		<form on-submit="submit">
			{{#user}}
				...
				<div class="row profile-edit__wrap">
					<div class="col-md-3">
						<div class="input input--grey">
							<input class="input__control datebirthdaypickr" name="birthdate" value="{{.birthdate}}" type="text" placeholder="Дата рождения" twoway="false">
