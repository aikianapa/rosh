        ...
        <div class="render">
          <div class="blog-inner__aside">
            <div wb-if="'{{_var.consult_text}}'>''">
              <p class="blog-inner__message">{{_var.consult_text}} <a href="tel:{{text2tel({{_var.consultPhone}})}}">{{_var.consultPhone}}</a>
              </p>
            ...
            <div class="recomendation" wb-if="'{{_var.recommend}}'!=='0'" data-s="{{_var.recommend}}">
              <a class="recomendation" href="/blog/{{wbFurlGenerate({{_var.recommend.header}})}}">
                ...
                <div class="recomendation__pic"><img src="{{_var.recommend.image}}" alt="{{_var.recommend.header}}">
                </div>
                <h4 class="h4 recomendation__title">{{_var.recommend.header}}</h4>
                <p class="recomendation__text text-small text-grey" wb-if="'{{_var.recommend.text}}'!==''">
                  {{wbGetWords({{_var.recommend.text}},10)}}</p>
              </a>
            </div>
          </div>
          ...
          <!--                        </wb-jq>-->
          <wb-jq
            wb-prepend=".blog-inner-content .render .container:not(.blog-image) .row:eq(0) .col-md-8:eq(0) .text:eq(0)">
          ...
          <div class="col-md-8">
            <div class="share">
              <div class="arrow__link" href="#">
                ...
                <a class="--openpopup" href="javascript:void(0)" data-popup="--fast"> Напишите нам </a>
              </div>
              <div class="arrow__link" href="#">
                <svg class="svgsprite _arrow-link">
                  <use xlink:href="/assets/img/sprites/svgsprites.svg#arrow-link"></use>
                </svg>
                <a href="tel:{{text2tel({{_var.contactPhone}})}}"> Позвоните нам </a>
              </div>
            </div>
          </div>
        </div>
...
<div class="to_top_btn">
  <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="20px"
       height="29px" viewBox="0 0 20 29" enable-background="new 0 0 20 29" xml:space="preserve">
            <path fill="#FFF"
