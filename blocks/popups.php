<view>
	...
	<div class="popup --record" id="newQuote">
		<div class="popup__overlay"></div>
		<template>
			<div class="popup__panel">
				<button class="popup__close">
	...
	<script type="wbapp">
        ...
        var newQuote = new Ractive({
            el: '#newQuote',
            template: $('#newQuote template').html(),
            data: {
            ...
            on: {
                init() {
                    setTimeout(function(){
                        wbapp.get('/api/v2/read/catalogs/srvcat', function(data) {
                            newQuote.set('categories', data);
                        });
                        wbapp.get('/api/v2/list/experts', function(data) {
                            newQuote.set('experts', data);
                        });
                        wbapp.get('/api/v2/list/services', function(data) {
                            newQuote.set('services', data);
                        });
                    },1500);
                },
                submit(ev) {
                    let $form = $(ev.node);
                    let uid = newQuote.get('user.id');
                    if ($form.verify() && uid > '') {
