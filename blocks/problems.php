	...
	<div class="container">
		...
		<div class="all-tabs-items">
			...
			<wb-foreach wb="table=catalogs&item=srvtype&from=tree.data">
				<div class="cursor-pointer all-tabs-item data-tab-link" data-srvtype="{{id}}" data-tabs="all-problems">{{name}}
				</div>
			</wb-foreach>
		</div>
	</div>
		...
		<div class="all-tab data-tab-item">
			<div class="container">
				<div class="all-services">
					<wb-var yonmap wb-api="/api/v2/list/_yonmap?f=problems" />
					<wb-var yonmap='{{array_column(_var.yonmap,"u","i")}}' />
					<wb-foreach wb="table=problems&sort=header&size=999999&tpl=false" wb-filter="active=on">
						...
						<a class="all-services__item" href="{{_var.yonmap.{{id}}}}" data-srvtype="{{srvtype}}" data-category='{{implode(" ",category)}}'>
