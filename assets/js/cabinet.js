...
function fix_comment(text) {
  ...
  tmp = tmp.replace(
    title_problems.toLowerCase(),
    '<div class="text-bold" style="color:#123">' +
      title_problems.toLowerCase() +
      "</div>",
  );
  tmp = tmp.replace(title_symptoms, title_symptoms.toLowerCase());
  tmp = tmp.replace(
    title_symptoms.toLowerCase(),
    '<div class="text-bold" style="color:#123">' +
      title_symptoms.toLowerCase() +
      "</div>",
  );
...
function unselectConsultation(el) {
  var _parent = $(el)
    .parents("form")
    .find(
      '.consultations .select-form[data-show="consultation-type"] .popups__text-chexboxs',
    );
...
$(function () {
  ...
  window.utils = {
    ...
    saveScroll(page_name) {
      ...
      window.onbeforeunload = function () {
        ...
        sessionStorage["state.tab-cabinet"] = $(
          ".account__tab.data-tab-item.active",
        ).data("tab");
        var _open = $(
          ".account__tab.data-tab-item.active " +
    ...
    restoreScroll(page_name, dont_clear) {
      ...
      if (_tab) {
        _el = $(
          '.account__tab-items .account__tab-item.data-tab-link[data-tab="' +
            _tab +
            '"]',
        );
        if (_el.length) {
    ...
    convertToMoscowTimezone(date) {
      const moscowTimezoneOffset = 3 * 60;
      const userTimezoneOffset = date.getTimezoneOffset();
      const timezoneDifference = userTimezoneOffset - moscowTimezoneOffset;
    ...
    formatTime(date, remove_seconds) {
      if (!!remove_seconds) {
        return this.getDate(date).toLocaleTimeString(navigator.language, {
          hour: "2-digit",
          minute: "2-digit",
        });
      }
  ...
  window.catalog = {
    ...
    init(only_clients) {
      ...
      if (only_clients) {
        utils.api
          .get("/api/v2/list/records?group=longterms&@group=client")
          .then(function (data) {
            if (data) {
        ...
        utils.api
          .get(
            "/api/v2/list/users?role=[main,expert,client]&active=on" +
          ...
          .then(function (data) {
            ...
            _self.users = utils.arr.indexBy(
              data["main"].concat(data["expert"]).concat(data["client"]),
            );
          });
      } else {
        _self.rawServices = [];
            ...
            .then(function (data) {
              ...
              keys.forEach(function (key) {
                ...
                if (cat.hasOwnProperty("children")) {
                  ...
                  _self.priceCategoriesOrder =
                    _self.priceCategoriesOrder.concat(_keys);
        ...
        getters.push(
          utils.api
            .get("/api/v2/list/price?active=on&@sort=header")
            .then(function (data) {
              if (!data) {
                return;
              }
        ...
        getters.push(
          utils.api
            .get(
              "/api/v2/list/users?role=expert&active=on" +
                "" +
                "&@sort=fullname:a",
            )
            .then(function (data) {
        ...
        getters.push(
          utils.api
            .get("/api/v2/list/_yonmap", { f: "users", "@return": "u,i" })
            .then(function (data) {
        ...
        return Promise.allSettled(getters).then(() => {
          _self.rawServices.forEach(function (service, i) {
            ...
            if (!service.hasOwnProperty("type")) {
              ...
              //);
              title =
                (!!service_parent.name /*&& (service.header.trim() != service_parent.name.trim())*/
                  ? service_parent.name + ": "
                  : "") + service.header;
              _cats = service.type;
            }
            ...
            let _item = {
              ...
              data: {
                ...
                service_title:
                  _self.priceCategories[service.category].name.trim() ||
                  title.trim(),
                tags: _tags,
                price: service.price,
                price_id: service.id,
          ...
          _self.priceCategoriesOrder.forEach(function (cid) {
            _self.servicesListOrderByCategories =
              _self.servicesListOrderByCategories.concat(
  ...
  window.Cabinet = {
    ...
    isCurrentDayEvent(event) {
      return (
        utils.formatDate(event.event_date) === utils.formatDate(new Date())
      );
    },
    ...
    updateProfile(profile_id, profile_data, callback) {
      ...
      utils.api
        .post("/api/v2/update/users/" + profile_id, data)
        .then(function (res) {
          if (!!callback) {
            callback(res);
          }
    ...
    updateQuote(record_id, record_data, callback) {
      ...
      utils.api
        .post("/api/v2/update/records/" + record_id, data)
        .then(function (res) {
          callback(res);
        });
    },
    listUpcomingEvents(options, callback) {
      ...
      utils.api
        .get("/api/v2/list/records?active=on&status=upcoming")
        .then(function (data) {
          data.forEach(function (rec) {
            ...
            if (
              event_from_timestamp < curr_timestamp &&
    ...
    getHistoryEvents(client_id, callback) {
      return utils.api
        .get("/api/v2/list/records?status=past&group=events")
        .then(function (data) {
    ...
    listLongterms(client_id) {
      return utils.api
        .get(
          "/api/v2/list/records?group=longterms&client=" +
  ...
  window.onlineRooms = {
    ...
    delete(meetingId, onSuccess) {
      var self = this;
      utils.api
        .post("/form/records/meetRoomDelete", { meetingId: meetingId })
        .then(function (data) {
          console.log("success:", data);
          onSuccess(data);
        });
  ...
  window.updPrice = function (_parent_form) {
    ...
    var sum = 0;
    _parent_form
      .find(".admin-editor__patient .search__drop-item.selected")
      .each(function (e) {
    ...
    _parent_form
      .find(".admin-editor__summ .price")
      .html(utils.formatPrice(sum) + " ₽<sup><b>*</b></sup>");
  ...
  window.initServicesSearch = function ($selector, service_list) {
    ...
    $(document).on("click", ".search__drop-delete:not(.fake)", function (e) {
      ...
      if (item.data("quote")) {
        _parent_form
          .find('input[name="for_consultation"]')
          .prop(":checked", true)
          .trigger("change");
      }
      item.remove();
    ...
    return $selector.autocomplete({
      ...
      beforeRender: function (container, suggestions) {
        ...
        var _catSelector = $(this)
          .parents("form")
          .find('[name="service_category"]:checked');
        var _selectedCat = "";
        ...
        $(suggestions).each(function (index) {
          ...
          $(this.data.tags).each(function () {
            ...
            TAGS.append(
              $("<div></div>")
                .addClass("search__drop-tag --" + this.color)
                .text(this.tag),
            );
          });
          ...
          CNT.append(
            $("<label></label>")
              .addClass("search__drop-item autocomplete-suggestion")
              .attr({
                "data-id": this.id,
                "data-index": index,
                "data-value": index,
              ...
              .append(
                $("<div></div>")
                  .addClass("search__drop-right")
                  .append(TAGS)
                  .append(
                    $("<div></div>")
                      .addClass("search__drop-summ")
                      .text(
                        (this.data.from == "on" ? "от  " : "") + PRICE + " ₽",
                      ),
                  ),
              ),
          );
      ...
      onSelect: function (suggestion) {
        ...
        if (
          _parent_form.find(
            '.admin-editor__patient:not(.price-list)[data-id="' +
              suggestion.id +
              '"]',
          ).length
        ) {
        ...
        var PRICE = new Intl.NumberFormat("ru-RU").format(
        ...
        const index = _parent_form.find(
          ".admin-editor__patient:not(.price-list) .search__drop-item[data-service_id]",
        ).length;
        var sum = 0;
        console.log("selected:", suggestion);
        if (suggestion.data.quote) {
          _parent_form
            .find('input.checkbox-visible-next-form[name="for_consultation"]')
            .prop("checked", true);
          _parent_form.find('[data-show="consultation-type"]').show();
          _parent_form
            .find(
              '[data-show-input="consultation-' + suggestion.data.quote + '"]',
            )
            .trigger("click");
          _parent_form
            .find(
              '.price-list .search__drop-item.consultation[data-consultation="' +
                suggestion.data.price_id +
                '"] label',
            )
            .trigger("click");
        ...
        _parent_form.find(".admin-editor__patient:not(.price-list)").append(
          $("<div></div>")
            .addClass("search__drop-item selected")
            .attr({
  ...
  window.initLongtermSearch = function ($form, for_client) {
    ...
    form.find(".event-search.longterm-search").autocomplete({
      ...
      serviceUrl: function () {
        ...
        return (
          "/api/v2/list/records?__token=" +
          wbapp._session.token +
          "&group=longterms" +
          client_qry
        );
      },
          ...
          suggestions: $.map(response, function (dataItem) {
            return {
              value:
                ...
                " " +
                dataItem.event_time_start +
                "-" +
                dataItem.event_time_end,
              data: dataItem.id,
            };
          }),
        };
    ...
    form.find(".event-search.record-search").autocomplete({
      ...
      serviceUrl: function () {
        ...
        return (
          "/api/v2/list/records?__token=" +
          wbapp._session.token +
          "&group=[longterms,events]&status=[upcoming,past,new,]" +
          client_qry +
          "&@sort=_lastdate:d"
        );
      },
        ...
        return {
          suggestions: $.map(response, function (dataItem) {
            ...
            return {
              ...
              data: {
                id: dataItem.id,
                is_longterm: dataItem.group === "longterms",
              },
            };
          }),
        };
  ...
  window.initClientSearch = function ($form) {
    ...
    window.client_search = form.find("input.client-search").autocomplete({
      ...
      serviceUrl:
        "/api/v2/list/users?role=client&active=on&@sort=fullname&__token=" +
        wbapp._session.token,
    ...
    return new Ractive({
      ...
      on: {
        ...
        keyup(ev) {
          ...
          var sms_code = "";
          $(".popup.--confirm-sms-code .code__input").each(
            function (digit, item) {
              sms_code += $(item).val();
            },
          );
      ...
      showPopup: function () {
        $(".popup.--confirm-sms-code .code__input").mask("9", {
          placeholder: "",
        });
      ...
      on: {
        ...
        submit(ev) {
          ...
          if ($(form).verify()) {
            ...
            utils.api.post(
              "/api/v2/" + (edit_mode ? "update" : "create") + "/records/",
              record_data,
              function (data) {
  ...
  window.changeLogSave = function (new_record_data, prev_record_data) {
    ...
    if (
      new_record_data.event_time_start != prev_record_data.event_time_start ||
      ...
      changelog.push({
        ...
        field: "event_time",
        prev_val:
          prev_record_data.event_time_start +
          " - " +
          prev_record_data.event_time_end,
        new_val:
          new_record_data.event_time_start +
          " - " +
          new_record_data.event_time_end,
      });
    }
    ...
    if (
      !!new_record_data?.services?.length ||
      !!prev_record_data?.services?.length
    ) {
      prev_labels = [];
      new_labels = [];
      if (!new_record_data?.services && !!prev_record_data.services) {
        ...
        new_record_data?.services?.join("") !=
        prev_record_data?.services?.join("")
      ) {
        prev_record_data?.services &&
          prev_record_data.services.forEach(function (val) {
            prev_labels.push(catalog.services[val].header);
          });
        new_record_data?.services &&
    ...
    if (
      !!new_record_data?.experts?.length ||
      !!prev_record_data?.experts?.length
    ) {
      ...
      if (!new_record_data?.experts && !!prev_record_data?.experts) {
        ...
        new_record_data?.experts?.join("") !=
        prev_record_data?.experts?.join("")
      ) {
        !!prev_record_data?.experts?.length &&
          prev_record_data.experts.forEach(function (expert_id) {
            console.log(catalog.experts[expert_id], expert_id);
        ...
        if (_files && _files.length > 0) {
          ...
          _files.forEach(function (file) {
            ...
            if (file.size >= max_size) {
              toast(
                "Размер файла c фото не может превышать 10 мб",
                "",
                "error",
              );
              $(this).val("");
